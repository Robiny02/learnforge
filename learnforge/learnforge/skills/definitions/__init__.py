...
RESUME_DIAGNOSIS_SKILL = SkillSpec(
    ...
    system_prompt=(
        ...
        "改写 rewritten_bullets 必须**信息密度高于原句、可直接替换进简历**：保留原 bullet 的关键技术信息"
        "（如 Manager/QA/Diagnosis/Planning/Mock/ReAct/唯一写入等），不许删成空泛短句，只收紧夸大措辞。\n"
    ...
    progressive_sections={
        "sop": (
            ...
            "3. 证据包：support_strength 按来源 none/doc_supported/code_supported/test_supported/"
            "runtime_supported（只读 README/CLAUDE.md → 最多 doc_supported）；missing_evidence 写『缺什么 + 去哪找』"
            "（architecture/implementation 缺的是源码/测试/trace/设计文档，不是性能指标；metric 才缺 QPS/延迟/准确率）；"
            "technical_highlight 写这条真正能打的工程点。\n"
